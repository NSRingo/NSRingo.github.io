...
 已通过推送安全响应更新  
禁止 MitM iOS 14 及以上版本`Siri 建议`  
`Siri 建议`配置下发所使用的`api.smoot.apple.com`的网络请求  
不再可以通过 MitM 改写  
...
## 解锁步骤
    ...
    <Steps>
      #### 第 1 步
      启用[`📍 定位`](../GeoServices/location) + [`🔍 聚焦搜索`](./spotlight)两个模块
      > [`📍 定位`](../GeoServices/location)的地区不可以设置为`🇨🇳CN`  
      > [`🔍 聚焦搜索`](./spotlight)的地区不可以设置为`🇨🇳CN`  
      > 可辅以`全局代理`确保稳定  
...
* 高级: 配合`BoxJs`及订阅使用
  ...
  2. 在`BoxJs`的配置面板中进行个性化设置:
     ...
     3. 在[`🔍 聚焦搜索`](http://boxjs.com/#/app/iRingo.Spotlight)根据需要填写您的设置信息
  </details>
